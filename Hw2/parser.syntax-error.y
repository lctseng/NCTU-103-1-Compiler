...
int_expr : arithmetic_expr
         ;

bool_expr : expr
          ;

expr : conditional_expr {printf("Reduce from conditional to expr\n");}
     ;

conditional_expr : conditional_expr OP_AND conditional_expr
                 | conditional_expr OP_OR conditional_expr
                 | OP_NOT conditional_expr
                 | relational_expr {printf("Reduce from relational to conditional\n");}
                 | '(' relational_expr ')' 
                 ;

relational_expr : arithmetic_expr OP_LE arithmetic_expr
                ...
                | arithmetic_expr %prec LOWER_THEN_ARITHMETIC_PARENTHESES {printf("Reduce from arithmetic to relational\n");}
                ;
...
arithmetic_expr : arithmetic_expr '+' arithmetic_expr
                ...
                | arithmetic_expr '/' arithmetic_expr
                | arithmetic_expr '%' arithmetic_expr
                | '-' arithmetic_expr %prec '*' {printf("Reduce with negation operator\n");}
                | '(' arithmetic_expr ')' 
                | literal_constant
                | funct_call {printf("Reduce from funct_call to arithmetic\n");}
                | var_ref {printf("Reduce from  var_ref to arithmetic\n");}
                ;




funct_call : scalar_id {printf("Func call see ()\n");} '(' zero_or_more_expr_list ')' {printf("Reduce to function call\n");}
           ;
